...
export const Rankings = ({
	...
	const aliveParticipants = sortedParticipants.filter(
		(participant) => participant && !participant.isDead,
	);

	const deadParticipants = sortedParticipants.filter(
		(participant) => participant?.isDead,
	);

	return (
		<div className="bg-card rounded-md p-4 flex flex-col h-full justify-between">
			{[...aliveParticipants, ...deadParticipants]?.map(
				(participant, index) => {
					if (!participant) {
						return null;
					}
					return (
						<div
							...
							className={cn(
								...
								participant?.isDead && "opacity-30",
							)}
						>
							<span className="text-lg font-extrabold w-[2ch] text-right mr-4">
								{index + 1}
							</span>
							<span className={cn("flex-1 text-2xl font-extrabold uppercase")}>
								{participant?.name}
							</span>
