...
const getFinalRankingsData = createServerFn({
  ...
  .handler(async ({ context }): Promise<FinalRankingsData> => {
    ...
    const participants = (await context.db.participants.find(() => true))
      .filter((p) => p.isCompeting && p.type === "FEATHERWEIGHT");
...
function RouteComponent() {
  ...
  return (
    <div className="p-6 flex flex-col gap-6 bg-zinc-900 rounded-xl shadow-lg">
      ...
      {hasDuplicates && (
        <div className="text-red-400 font-semibold">Duplicate selections detected. Ensure each bot is selected only once.</div>
      )}
